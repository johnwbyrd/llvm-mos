...
MOSGDBRemoteRegisterContext::MOSGDBRemoteRegisterContext(
  ...
  LLDB_MOS_LOG_REG(
      "[MOSGDBRemoteRegisterContext ctor] this={0}, map addr={1}, map size={2}",
...
bool MOSGDBRemoteRegisterContext::IsImaginaryRegister(
  ...
  if (sname.size() > 2 && sname.substr(0, 2) == "rc") {
    ...
    return imaginary_register_map_.count(rc_lo) > 0 &&
           imaginary_register_map_.count(rc_hi) > 0;
  }
  return false;
}
...
bool MOSGDBRemoteRegisterContext::ReadRegister(
  ...
  LLDB_MOS_LOG_REG("[ReadRegister] RC this={0}, map addr={1}, map size={2}",
                   (void *)this, (void *)&imaginary_register_map_,
                   imaginary_register_map_.size());
  const char *name = reg_info->name;
  if (!name)
  ...
  if (sname.size() > 2 && sname.substr(0, 2) == "rc") {
    ...
    LLDB_MOS_LOG_REG(
        "ReadRegister: lookup SUCCEEDED for key '{}', value=0x{:x}", sname,
        it->second);
    lldb_private::Status error;
    uint8_t byte = 0;
    size_t bytes_read =
        m_thread.GetProcess()->ReadMemory(it->second, &byte, 1, error);
    if (bytes_read == 1 && error.Success()) {
      value.SetUInt(byte, 1);
      return true;
    }
    LLDB_MOS_LOG_REG("Failed to read memory for {} at address 0x{:x}", sname,
                     it->second);
    return false;
  } else if (sname.size() > 2 && sname.substr(0, 2) == "rs") {
    int n = std::atoi(sname.c_str() + 2);
    ...
    if (it_lo == imaginary_register_map_.end() ||
        it_hi == imaginary_register_map_.end()) {
      LLDB_MOS_LOG_REG(
          "Failed to find {} or {} in imaginary_register_map_ for {}", rc_lo,
          rc_hi, sname);
      return false;
    }
    lldb_private::Status error;
    uint8_t lo = 0, hi = 0;
    LLDB_MOS_LOG_REG("Reading rs* register {} from rc_lo=0x{:x}, rc_hi=0x{:x}",
                     sname, it_lo->second, it_hi->second);
    if (m_thread.GetProcess()->ReadMemory(it_lo->second, &lo, 1, error) != 1 ||
        !error.Success()) {
      LLDB_MOS_LOG_REG("Failed to read memory for {} (lo) at address 0x{:x}",
                       rc_lo, it_lo->second);
      return false;
    }
    if (m_thread.GetProcess()->ReadMemory(it_hi->second, &hi, 1, error) != 1 ||
        !error.Success()) {
      LLDB_MOS_LOG_REG("Failed to read memory for {} (hi) at address 0x{:x}",
                       rc_hi, it_hi->second);
      return false;
    }
...
bool MOSGDBRemoteRegisterContext::WriteRegister(
  ...
  if (sname.size() > 2 && sname.substr(0, 2) == "rc") {
    ...
    size_t bytes_written =
        m_thread.GetProcess()->WriteMemory(it->second, &byte, 1, error);
    ...
    lldb_private::Status error;
    if (m_thread.GetProcess()->WriteMemory(it_lo->second, &lo, 1, error) != 1 ||
        !error.Success())
      return false;
    if (m_thread.GetProcess()->WriteMemory(it_hi->second, &hi, 1, error) != 1 ||
        !error.Success())
      return false;
    return true;
  }
