...
BitVector MOSRegisterInfo::getReservedRegs(const MachineFunction &MF) const {
  ...
  // Reserve RS10 as a dedicated scratch for the soft-stack STStk/LDStk pseudos
  // in reentrant functions. This must be a register that is never chosen as
  // the destination/source of STStk/LDStk itself; otherwise the pseudo would
  // have two aliased defs (dst and early-clobber scratch) at the same slot,
  // which fails MI verification.
  //
  // RS8 (the scavenger's save area for A/Y/P via RC16/RC17) and RS9 (the
  // indirect-call callee register per MOSCallLowering) both have existing
  // roles that make them unsafe: reserving RS8 collides with the scavenger
  // (see the reverted PR #540 in llvm-mos), and reserving RS9 collides with
  // spilling the indirect-call callee (creates the aliased-def MI). RS10 is
  // callee-saved (see MOSCallingConv.td) and has no other explicit
  // physical-register use, so it can be safely reserved and clobbered as
  // scratch; PEI saves and restores it via the standard callee-save
  // machinery because RC20/RC21 are on MOS_CSR.
  //
  // Static-stack functions never emit STStk/LDStk, so this reservation is
  // unnecessary there.
  if (!TFI->usesStaticStack(MF))
    reserveAllSubregs(&Reserved, MOS::RS10);
  return Reserved;
}
