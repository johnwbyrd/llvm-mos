...
public:
  ...
  static std::unique_ptr<MOSOperand> createImm(const MCSubtargetInfo &STI,
                                               const MCExpr *Imm, SMLoc S,
                                               SMLoc E) {
...
#include "MOSGenAsmMatcher.inc"
  ...
  ParseStatus tryParseRegister(MCRegister &Reg, SMLoc &StartLoc,
    ...
    if (Reg == 0 &&
        !getSTI().hasFeature(MOS::FeatureAltRegisterNamesOnly)) {
    ...
    while (!Parser.getTok().is(AsmToken::EndOfStatement) &&
      ...
      if (isSPC700()) {
        ...
        if (Parser.getTok().is(AsmToken::Dot)) {
          ...
          if (parseExpr(Operands, Ctx_Other,
      ...
      if (ExpectedClose != AsmToken::Eof &&
          Parser.getTok().is(ExpectedClose)) {
        consumeToken(Operands);
  ...
  void reportNearMisses(SmallVectorImpl<NearMissInfo> &NearMisses,
